...
class MultiFileWriter:
    ...
    def __init__(self, base_dir, name, bucket_name):
        ...
        self._name = name
        self._file_gen = (open(self._base_dir / f'{name}_{i:09}.bin', 'wb')
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = open(f'/home/maayana14/postings_gcp/{f_name}', 'rb')
...
class InvertedIndex:
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name):
        with open(f"{bucket_id}_posting_locs_t.pickle", "wb") as f:
            pickle.dump(posting_locs, f)
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        blob_posting_locs = bucket.blob(f"postings_gcp/{bucket_id}_posting_locs_t.pickle")
        blob_posting_locs.upload_from_filename(f"{bucket_id}_posting_locs_t.pickle")

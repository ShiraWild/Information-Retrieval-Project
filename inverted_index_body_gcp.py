    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = open(f'/home/maayana14/postings_body/{f_name}', 'rb')
...
class InvertedIndex:
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name):
        with open(f"{bucket_id}_posting_locs_b.pickle", "wb") as f:
            pickle.dump(posting_locs, f)
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        blob_posting_locs = bucket.blob(f"postings_body/{bucket_id}_posting_locs_b.pickle")
        blob_posting_locs.upload_from_filename(f"{bucket_id}_posting_locs_b.pickle")
